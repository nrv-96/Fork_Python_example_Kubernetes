...
HOOK_LIST = [
    "ccplatcfnginlibs.hooks.post_build.publish_params",
    "ccplatcfnginlibs.hooks.custom_parameter_create.hook",
]


def hook(provider, context, **kwargs):
    try:
        logger.info(f'Executing post-build hooks for module: {str(context.environment["module_name"])}')
